...
bool cubicSpline::loadData(double *x_data, double *y_data, int count, double bound1, double bound2, BoundType type) {
    if ((x_data == NULL) || (y_data == NULL) || (count < 3) || (type > BOUNDTYPE_SECOND_DERIVATIVE) || (type < BOUNDTYPE_FIRST_DERIVATIVE)) {
        return false;
    }
    initParam();
...
bool cubicSpline::spline(BoundType type) {
    ...
    else if(type == BOUNDTYPE_SECOND_DERIVATIVE) {
        d[1]=d[1]-a[1]*f1;
...
RobotControl :: RobotControl(std::string name) : Node(name) {
    this->declare_parameter<int>("arm_type", arm_type);
    this->get_parameter("arm_type", arm_type);

    this->declare_parameter<bool>("follow", follow);
    this->get_parameter("follow", follow);
    ...
    timer_ = this->create_wall_timer(std::chrono::milliseconds(20), std::bind(&RobotControl::timer_callback, this));
    this->action_server = rclcpp_action::create_server<FollowJointTrajectory>(
                this, "/z1_group_controller/follow_joint_trajectory",
                std::bind(&RobotControl::handle_goal, this, _1, _2),
                std::bind(&RobotControl::handle_cancel, this),
                std::bind(&RobotControl::handle_accepted, this, _1));

    joint_pos_pub = this->create_publisher<robot_interfaces::msg::Jointpos>("/rm_driver/movej_canfd_cmd", 10);
}
...
rclcpp_action::CancelResponse RobotControl::handle_cancel()
{
...
void RobotControl::handle_accepted(const std::shared_ptr<GoalHandleFJT> goal_handle) 
{
    using std::placeholders::_1;

    std::thread{std::bind(&RobotControl::execute_move, this, _1), goal_handle}
    .detach();
}

/* 收到action的goal后调用的回调函数 */
void RobotControl::execute_move(const std::shared_ptr<GoalHandleFJT> goal_handle)
{
    int i = 0;
    ...
    if  (point_num > 3) //判断当moveit规划的路点数大于3时为有效规划并进行三次样条插值
    {
        ...
        /* 各个关节位置 */
        double *p_joint1 = new double[point_num];
        double *p_joint2 = new double[point_num];
        double *p_joint3 = new double[point_num];
        double *p_joint4 = new double[point_num];
        double *p_joint5 = new double[point_num];
        double *p_joint6 = new double[point_num];
        double *p_joint7 = new double[point_num];

        /* 各个关节速度 */
        double *v_joint1 = new double[point_num];
        double *v_joint2 = new double[point_num];
        double *v_joint3 = new double[point_num];
        double *v_joint4 = new double[point_num];
        double *v_joint5 = new double[point_num];
        double *v_joint6 = new double[point_num];
        double *v_joint7 = new double[point_num];

        /* 各个关节加速度 */
        double *a_joint1 = new double[point_num];
        double *a_joint2 = new double[point_num];
        double *a_joint3 = new double[point_num];
        double *a_joint4 = new double[point_num];
        double *a_joint5 = new double[point_num];
        double *a_joint6 = new double[point_num];
        double *a_joint7 = new double[point_num];

        /* 时间数组 */
        double *time_from_start = new double[point_num];
        double *timens_from_start = new double[point_num];
    ...
    if(point_changed)
    {
        if(p2.vector_cnt < p2.vector_len)
        {
            ...
            if(arm_type == 75)
            {
                ...
                this->joint_pos_pub->publish(joint_msg);
            }
            else
            {
                joint_msg.joint[0] = p_joint1_.at(p2.vector_cnt);
...
int main(int argc, char const *argv[])
{
    ...
    rclcpp::spin(std::make_shared<RobotControl>("robot_control"));
    rclcpp::shutdown();
    return 0;
}
